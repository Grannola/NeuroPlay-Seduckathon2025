...
window.resposta_correta;
function gerarConta() {
    let valor1 = Math.floor(Math.random() * 100) + 1;
    let valor2 = Math.floor(Math.random() * 100) + 1;
    
    document.getElementById('conta').textContent = `${valor1} + ${valor2} = ?`
    window.resposta_correta = valor1 + valor2;
}

function verificar() {
    let numero = Number(document.getElementById('resposta').value);
    if (numero === window.resposta_correta) {
